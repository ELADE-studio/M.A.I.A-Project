...
function listen() {
  ...
  recognition.onresult = function (event) {
    ...
    .then(response => {
      if (response.status === 429) { // Check if the response status is 429
        throw new Error("ERROR 1 : I'm sorry, but you have already used all your API Tokens.");
      } else if (!response.ok) { // Check if the response status is not ok
        throw new Error("ERROR 2 : I'm sorry, but I'm having trouble with your API key. Are you sure you've registered it in the right place and that it's valid?");
      } else {
